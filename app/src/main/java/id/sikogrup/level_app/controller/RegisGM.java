...
public class RegisGM extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_regis_gm);
        ...
        buttonRegister.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (validate()) {
                    ...
                    Query usernameQuery = FirebaseDatabase.getInstance().getReference().child("Users").child("GuildMaster").orderByChild("username").equalTo(UserName);
                    usernameQuery.addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            if(dataSnapshot.getChildrenCount()>0){
                                Toast.makeText(RegisGM.this, "Choose a different Username!", Toast.LENGTH_SHORT).show();
                            }
                            else{
                                //Check in the database is there any user associated with  this email
                                mAuth.createUserWithEmailAndPassword(Email,Password).addOnCompleteListener(RegisGM.this, new OnCompleteListener<AuthResult>() {
                                    @Override
                                    public void onComplete(@NonNull Task<AuthResult> task) {
                                        if(!task.isSuccessful()){
                                            Toast.makeText(RegisGM.this, "Register Failed= "+task.getException(), Toast.LENGTH_SHORT).show();
                                        }
                                        else {
                                            String user_id = mAuth.getCurrentUser().getUid();
                                            DatabaseReference database = FirebaseDatabase.getInstance().getReference().child("Users").child("GuildMaster").child(user_id);
                                            GuildMaster user = new GuildMaster(UserName,Email,fullName,phone,user_id);
                                            database.setValue(user);

                                            Toast.makeText(RegisGM.this, "Register Successful", Toast.LENGTH_SHORT).show();
                                            resetView();
    ...
    private void initTextViewLogin() {
        ...
        textViewLogin.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(RegisGM.this, LoginGM.class);
